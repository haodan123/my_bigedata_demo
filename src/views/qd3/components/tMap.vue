<template>
  <div class="map_box">
    ...
    <img src="@/assets/qd4/map_border.png" class="map_border" />
...
const initMap = () => {
  ...
  // tdtMap.value.centerAndZoom(new window.T.LngLat(119.93428,36.13166), 17)
  // 出场动画
  // tdtMap.value.centerAndZoom(new window.T.LngLat(119.9336, 36.12998), 10)
  tdtMap.value.centerAndZoom(new window.T.LngLat(119.93312,36.12738), 10)
  timer.value = setInterval(() => {
    if (tdtMap.value.getZoom() < 16) {
      tdtMap.value.zoomIn()
    }
    if (tdtMap.value.getZoom() == 16) {
...
const addRectangle = (points, content, color = '#12af82', borderColor) => {
  ...
  const infoWindowHtml = `
        <div class="custom-info-window">
          <div class='tc_item'> 
              <div>地块名称</div>:
              <div class="text_color">${'肉牛'}养殖场</div>
            </div>
            <div class='tc_item'> 
              <div>地块负责人</div>:
...
.overlay {
  ...
  pointer-events: none;
  z-index: 13;
  // background: url('@/assets/global/black_border.png') no-repeat 100% 100%;
