<template>
  <div class="map_box">
    ...
    >
      <div class="dialogBox">
        <boxContainer_nncs bigContainer title="产品介绍">
          <div class="containerBox">
            <div class="richbox" v-html="dialogData.context"></div>
            <!-- <el-image style="width: 100px; height: 100px" :src="dialogData.picture" fit="cover" /> -->
          </div>
        </boxContainer_nncs>
      </div>
    </el-dialog>
...
import qyicon from '@/assets/mapImg/qyicon.png'
import boxContainer_nncs from '@/components/boxContainer_nncs/index.vue'
import { onMounted, ref } from 'vue'
const emit = defineEmits(['openThePop'])
...
const initMap = () => {
  ...
  // 出场动画
  tdtMap.value.centerAndZoom(new window.T.LngLat(102.74895, 27.04009), 10)
  ...
  qxcgqList.forEach(item => {
    addMarker(item.lnglat, item, qyicon)
  })
...
const getData = async () => {
  const { data: res } = await requestGet(`qyxx`)
  res.forEach(item => {
 ...
 */
const addMarker = (lnglat, infoContent, imgUrl, iconParms = { url: qyicon, size: [30, 30], off: [15, 40] }) => {
  //创建图片对象
  ...
  const infoWindowHtml = `
        <div class="custom-info-window">
            <div class='tc_item'> 
              <div>加工厂名称</div>:
              <div class="text_color">${infoContent.jname || '暂无'}</div>
            </div>
            <div class='tc_item'> 
              <div>加工厂类型</div>:
              <div class="text_color">${infoContent.jtype || '暂无'}</div>
            </div>
            <div class='tc_item'> 
              <div>负责人</div>:
              <div class="text_color">${infoContent.person || '暂无'}</div>
            </div>
            <div class='tc_item'> 
              <div>电话号码</div>:
              <div class="text_color">${infoContent.phone || '暂无'}</div>
            </div>
            <div class='tc_item'> 
              <div>面积</div>:
              <div class="text_color">${infoContent.area+'亩' || '暂无'}</div>
            </div>
            <div class='tc_item'> 
              <div>地址</div>:
              <div class="text_color">${infoContent.address || '暂无'}</div>
            </div>
  ...
  marker.addEventListener('click', () => {
    // console.log('打开弹窗')
    // emit('openThePop', infoContent)
    dialogVisible.value = true
    dialogData.value = infoContent
  })
...
.tdt-infowindow-content-wrapper {
  min-width: 200px;
  min-height: 100px;
  background: url('@/assets/mapImg/tan_box.png') center center no-repeat;
  background-size: 100% 100%;
  box-shadow: none;
  overflow: hidden;
}
